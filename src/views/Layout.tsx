...
class Layout extends React.Component<any, any> {
	...
	loadProjectFromStateChange(path?:string) {
		...
		let m:any, selectedProjectName:string = ""
	...
	navItem = (props:any) => {
		return (
			<li onClick={this.onNavClick} className={(props.className||"") + (isActive(props.path) ? " active" : "")}>
	...
	projects = () => {
		return Persistence.instance().getProjectNames().map(name => { return {
			value: name,
			label: name
		}})
	}
	...
	getSelectedProject = () => {
		if (this.state.selectedProjectName === "" || this.state.selectedProjectName === undefined) {
			return undefined
		}

		let selected = this.projects().find(p => p.value === this.state.selectedProjectName)
	...
	load = () => {
		...
		if (!this.state.selectedProjectName) {
			this.setState({
				...
				this.props.history.push("/new-project/"+this.state.selectedProjectName)
			})
		} else {
			this.props.history.push("/new-project/"+this.state.selectedProjectName)
		}
	...
	delete = () => {
		...
		this.props.history.push("/new-project/MyProject")
	}
		...
		return (
			this.ready && <div>
				<nav className="navbar navbar-inverse navbar-fixed-top">
					<div className="container">
						<div className="navbar-header">
							<NavLink className="navbar-brand" to="/" activeClassName="active" isActive={isActive}>
								FFMpeg UI
							</NavLink>
						</div>
						<div className="collapse navbar-collapse">
							<ul className="nav navbar-nav pull-right">
								<NavItem className="hidden new-project-link" title="New project" path={"/new-project/" + this.state.selectedProjectName}></NavItem>
								<NavItem title="About" path="/about"></NavItem>
							</ul>
							<form className="navbar-form navbar-right">
								...
								<div className="btn-group">
									...
									<Tooltip content="Create a new project or load an existing one" placement="bottom">
										<button disabled={this.onProjectPage() && (this.state.loadedProjectName == this.state.selectedProjectName)} 
											type="button" 
									...
									<Tooltip content="Save the current project to the local storage (without media)" placement="bottom">
										<button disabled={!this.onProjectPage() || !this.state.loadedProjectName} 
											type="button" 
									...
									<Tooltip content="Delete the current project from the local storage" placement="bottom">
										<button disabled={!this.onProjectPage() || !this.state.loadedProjectName} 
										type="button" 
				...
				<section className="container">
					...
					{this.state.loadedProjectName && <Route path="/new-project/:name" exact component={WorkFlow} />}
